...
class TestFrontPortRearPortsSerialization(BaseTest):
    """End-to-end coverage for FrontPort.rear_ports (NetBox 4.5+ list-of-mapping
    field, no `id` on each item). Validates:

    - Issue #745: re-fetched FrontPort doesn't produce a false-positive diff
      and save() is a no-op when nothing changed.
    - PR #769: in-place mutation of a rear_ports item is detected by _diff()
      and persisted by save().
    """

    @pytest.fixture(scope="class")
    def device(self, api, device_type, role, site, nb_version):
        if version.parse(str(nb_version)) < version.parse("4.5"):
            pytest.skip("FrontPort.rear_ports list field requires NetBox 4.5+")
        from tests.integration.conftest import create_device

        device = create_device(api, site, device_type, role, "test-device-fp-rp")
        yield device
        device.delete()

    @pytest.fixture(scope="class")
    def rear_port(self, api, device):
        ret = api.dcim.rear_ports.create(
            name="MutRearPort", device=device.id, type="8p8c", positions=4
        )
        yield ret

    @pytest.fixture(scope="class")
    def front_port(self, api, device, rear_port):
        ret = api.dcim.front_ports.create(
            name="MutFrontPort",
            device=device.id,
            type="8p8c",
            rear_ports=[
                {"rear_port": rear_port.id, "rear_port_position": 1, "position": 1},
            ],
        )
        yield ret

    @pytest.fixture(scope="class")
    def init(self, request, front_port):
        self._init_helper(
            request,
            front_port,
            filter_kwargs={"name": "MutFrontPort"},
            endpoint="front_ports",
        )

    def test_save_unchanged_is_noop(self, api, front_port):
        # Issue #745 regression: refetch and immediately save — no false-positive
        # diff should be generated by the list-of-mapping rear_ports field.
        refetched = api.dcim.front_ports.get(front_port.id)
        assert refetched._diff() == set()
        assert refetched.save() is False

    def test_mutate_rear_port_position_persists(self, api, front_port):
        # PR #769 regression: an in-place mutation of an id-less nested
        # rear_ports item must be detected by _diff() and persisted by save().
        refetched = api.dcim.front_ports.get(front_port.id)
        assert refetched.rear_ports[0].rear_port_position == 1

        refetched.rear_ports[0].rear_port_position = 2
        assert refetched._diff() == {"rear_ports"}
        assert refetched.save() is True

        again = api.dcim.front_ports.get(front_port.id)
        assert again.rear_ports[0].rear_port_position == 2
